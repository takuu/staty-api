...
exports.create = function (req, res) {
  ...
  var player = _req$body.player;
  var game = _req$body.game;
...
exports.getPlayerStats = function (req, res) {
  ...
  Stat.find({ player: id }).populate('vsTeam game').populate('league', 'name _id').populate('season', 'name _id active').exec(function (err, stats) {
    res.status(200).send(stats);
  });
};
...
exports.getPlayerListStats = function (req, res) {
  ...
  var list = _.map(id.split(','), function (item) {
    return ObjectId(item);
  });
  console.log('test: ', list.length, list);
  if (list && list.length) {
    Stat.find({ 'player': { $in: list } }).populate('division', 'name _id').populate('league', 'name _id').populate('season', 'name _id active').populate('team vsTeam game player').populate({
      path: 'player',
...
exports.getTeamStats = function (req, res) {
  var id = req.params.id;
  Stat.find({ team: id }).populate('team vsTeam game player').exec(function (err, stats) {
    res.status(200).send(stats);
  });
};
...
exports.getGameStats = function (req, res) {
  var id = req.params.id;
  Stat.find({ game: id }).populate('player game').exec(function (err, stats) {
    res.status(200).send(stats);
  });
};
...
//# sourceMappingURL=stat.controller-compiled.js.map